...
// botImpl is the Telegram Bot API client.
type botImpl struct {
	token      string
	apiURL     string
...
func (b *botImpl) SetAPIURL(url string) {
	b.apiURL = url
}

// SetWebhook registers the webhook URL with Telegram.
func (b *botImpl) SetWebhook(webhookURL string) error {
	...
	if err != nil {
		return fmt.Errorf("pkg: failed to set webhook: %w", err)
	}
	defer resp.Body.Close()

	var apiResp APIResponse
	if err := json.NewDecoder(resp.Body).Decode(&apiResp); err != nil {
		return fmt.Errorf("pkg: failed to decode webhook response: %w", err)
	}
	if !apiResp.OK {
		return fmt.Errorf("pkg: telegram setWebhook failed: %s", apiResp.Description)
	}
	return nil
}
...
func (b *botImpl) SendMessageWithMode(chatID int64, text string, parseMode string) error {
	...
	if err != nil {
		return fmt.Errorf("pkg: failed to marshal message: %w", err)
	}

	resp, err := b.httpClient.Post(url, "application/json", bytes.NewBuffer(body))
	if err != nil {
		return fmt.Errorf("pkg: failed to send message: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		raw, _ := io.ReadAll(resp.Body)
		return fmt.Errorf("pkg: telegram sendMessage API error %d: %s", resp.StatusCode, string(raw))
	}
...
func (b *botImpl) SendMessageHTML(chatID int64, text string) error {
	url := fmt.Sprintf("%s/sendMessage", b.apiURL)
...
func (b *botImpl) SendMessagePlain(chatID int64, text string) error {
	url := fmt.Sprintf("%s/sendMessage", b.apiURL)
