...
// gcalendarImpl wraps the Google Calendar API service.
type gcalendarImpl struct {
	service *calendar.Service
}

var _ IGCalendar = (*gcalendarImpl)(nil)

// NewClientFromCredentialsFile creates a Calendar client from a Service Account JSON file path.
func NewClientFromCredentialsFile(ctx context.Context, credentialsPath string) (IGCalendar, error) {
	data, err := os.ReadFile(credentialsPath)
	if err != nil {
		return nil, fmt.Errorf("pkg: failed to read credentials file: %w", err)
	}
...
func NewClientFromCredentialsJSON(ctx context.Context, credentialsJSON []byte) (IGCalendar, error) {
	...
	if err == nil {
		...
		if svcErr != nil {
			return nil, fmt.Errorf("pkg: failed to create calendar service: %w", svcErr)
		}
		return &gcalendarImpl{service: svc}, nil
	}
	...
	if jsonErr := json.Unmarshal(credentialsJSON, &oauthCreds); jsonErr != nil {
		return nil, fmt.Errorf("pkg: unsupported credentials format: %w", jsonErr)
	}
	...
	if tokenErr != nil {
		return nil, fmt.Errorf("pkg: google credentials are OAuth Desktop type but no token.json found: use Service Account instead")
	}

	var tok oauth2.Token
	if jsonErr := json.Unmarshal(tokenData, &tok); jsonErr != nil {
		return nil, fmt.Errorf("pkg: failed to parse token.json: %w", jsonErr)
	}

	tokenSource := oauthConfig.TokenSource(ctx, &tok)
	svc, svcErr := calendar.NewService(ctx, option.WithTokenSource(tokenSource))
	if svcErr != nil {
		return nil, fmt.Errorf("pkg: failed to create calendar service from OAuth token: %w", svcErr)
	}
...
func NewClientFromHTTP(ctx context.Context, httpClient *http.Client) (IGCalendar, error) {
	svc, err := calendar.NewService(ctx, option.WithHTTPClient(httpClient))
	if err != nil {
		return nil, fmt.Errorf("pkg: failed to create calendar service: %w", err)
	}
	return &gcalendarImpl{service: svc}, nil
}

// CreateEvent creates a new Google Calendar event.
func (c *gcalendarImpl) CreateEvent(ctx context.Context, req CreateEventRequest) (*Event, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pkg: failed to create calendar event: %w", err)
	}
...
func (c *gcalendarImpl) ListEvents(ctx context.Context, req ListEventsRequest) ([]Event, error) {
	calendarID := req.CalendarID
	if calendarID == "" {
		calendarID = "primary"
	}
	...
	if err != nil {
		return nil, fmt.Errorf("pkg: failed to list calendar events: %w", err)
	}
